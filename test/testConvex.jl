...
f2(x) = @. (2)*x[1]^2 + (5)*x[1] + 3 + (2)*abs(x[1] - 2)
xL = [-3.0]
xU = [3.0]

_, bcoeff1, ccoeff1 = eval_sampling_underestimator_coeffs(f2, xL, xU)
...
_, bcoeff2, ccoeff2 = eval_sampling_underestimator_coeffs(f3, xL, xU)
...
_, bcoeff3, ccoeff3 = eval_sampling_underestimator_coeffs(f5, xL, xU)
