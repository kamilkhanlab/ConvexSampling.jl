...
@testset "Example 1: A function with 1 variable:" begin
    ...
    xL = [-3.0]
    xU = [3.0]
    @test isapprox(eval_sampling_underestimator_coeffs(f1, xL, xU),
        ([0.0], [3.0000], 6.8200, Array{Float64}(undef, 1, 1)))
...
@testset "Example 2: A function with 2 variables:" begin
    ...
    xL = [-5.0, -2.0]
    xU = [5.0, 3.0]
    # @test isapprox(eval_sampling_underestimator_coeffs(f2, xL, xU),
...
@testset "Example 3: A function with 3 variables:" begin
    ...
    xL = [-5.0, -2.0, -1.0]
    xU = [6.0, 3.0, 7.0]
    # @test isapprox(eval_sampling_underestimator_coeffs(f3, xL, xU),
